...
class AiTaFeedbackJoiner:
    def __init__(self, ai_feedback_path=AI_FEEDBACK_PATH, ta_feedback_path=TA_OVERRIDES_PATH,
        ...
        self.fieldnames = ['course_id', 'course_name', 'assignment_id', 'task_id', 'student_id', 'task_title', 'grade_difference', 'ai_feedback',
                           'ta_feedback', 'feedback_additive_differential', 'feedback_additive_embedding',
                           'feedback_subtractive_differential', 'feedback_subtractive_embedding', 'ta_id', 'ta_name']
    ...
    def build_feedback_differential_table(self):
        ...
        for task_id in tqdm(joined_feedback_df['task_id'].unique()):
            ...
            for (student_id), student_response_group in filtered_df.groupby(['student_id']):
                ...
                ai_feedbacks = []
                feedback_part_names = []
                feedback_subtractive_differential = ""
                ...
                for (part_name), student_part_response_group in student_response_group.groupby(['part_name']):
                    ...
                    feedback_subtractive_differential += calculate_subtractive_feedback_differential(single_ai_feedback, ta_feedback)
                ...
                new_row = {'course_id': student_response_group['course_id'].iloc[0],
                           ...
                           'feedback_subtractive_differential': feedback_subtractive_differential,
...
def add_embeddings_if_needed(existing_data_path, new_data_row, existing_data_exists):
    if existing_data_exists:
        ...
        if not existing_row.empty:  # If a matching row is found, use its embeddings
            ...
            new_data_row['feedback_subtractive_embedding'] = existing_row['feedback_subtractive_embedding'].iloc[0]
...
def calculate_subtractive_feedback_differential(ai_feedback, ta_feedback):
    ai_sentences = set(re.split(r'(?<!\d)[.!?](?=\s+|[A-Z]|$)|\n+', ai_feedback))
    ta_sentences = set(re.split(r'(?<!\d)[.!?](?=\s+|[A-Z]|$)|\n+', ta_feedback))
    ...
    # Check if there are any common sentences between AI feedback and TA feedback
    if '' in ai_sentences:
        ai_sentences.remove('')

    if not ta_sentences.intersection(ai_sentences):
        return ""

    # Find sentences in AI feedback that were removed by TA
    subtractive_sentences = [sentence for sentence in ai_sentences if sentence not in ta_feedback]
    subtractive_feedback_differential = ' '.join(subtractive_sentences)
    return subtractive_feedback_differential
